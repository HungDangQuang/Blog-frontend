...
import { getUserInfo, deleteUser } from "../apis/user";
...
function UserTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      let data = await getUserInfo(); // params in url
      getUsers(data);
    };
    fetchData();
  }, []);
  ...
  const handleDelete = (id) => {
    ...
    getUsers(users.filter((item) => item._id !== id));
    deleteUser(id);
    setNotify({
      isOpen: true,
